...
- Modification date: 2 March 2015
...
and a `src/Makevars` file with the following lines
    ...
    RHTSLIB_LIB=`echo 'Rhtslib::pkgconfig("PKG_LIBS")'|\
        "${R_HOME}/bin/R" --vanilla --slave)`
    PKG_LIBS=$(RHTSLIB_LIBS)
...
to the `PKG_LIBS=$(RHTSLIB_LIBS)` line, e.g.,
    ...
    PKG_LIBS=$(RHTSLIB_LIBS) -L/path/foo -lbar
...
If `LinkingTo` is not available to you, add to `src/Makevars.win` the
following lines
...
and to `src/Makevars` the following lines

    RHTSLIB_CPPFLAGS=`echo 'Rhtslib::pkgconfig("PKG_CPPFLAGS")'|\
        "${R_HOME}/bin/R" --vanilla --slave`
    PKG_CPPFLAGS=$(RHTSLIB_CPPFLAGS)

As with `$PKG_LIBS`, if your package must add to the `$PKG_CPPFLAGS`
variable, do so by appending to the `PKG_CPPFLAGS` line.
